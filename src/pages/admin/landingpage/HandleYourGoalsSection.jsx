...
const HandleYourGoalsSection = () => {
  ...
  const handleEdit = (goal) => {
    ...
    setFormData({
      ...
      status: goal.status === 1 || goal.status === true || goal.status === "1" ? true : false
    });
    setFormErrors({});
    setShowForm(true);
  ...
  const handleStatusToggle = async (goal) => {
    try {
      const response = await api.patch(`/admin/your-goals-sections/${goal.id}/status`, {
        status: !(goal.status === 1 || goal.status === true || goal.status === "1") ? 1 : 0
      });
      if (response.data?.status) {
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all flex items-center gap-1.5 ${
                            goal.status === 1 || goal.status === true || goal.status === "1"
